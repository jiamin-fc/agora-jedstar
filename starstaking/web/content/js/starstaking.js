...
var populated = false;
var contract_address = "0x311ecCc6d41ECa8eA2cD10339497c71091086220";
var interval={};
var isTVL = false;
var mybalance;
        ...
        Promise.all([
            ...
            updateStakeable().then(function(res){
                for (k=0; k < res.length; k++){
                    total[k+1] = res[k]["total"];
                    stakeable[k+1] = res[k]["total"] - res[k]["staked"];
                    staked[k+1] = res[k]["staked"];
                    document.getElementById("stakeable"+(k+1)).innerHTML = formatNum(stakeable[k+1]) + " JED";
                }   
            }).catch(function(err){
            ...
            Promise.all([populateCard(1), populateCard(2), populateCard(3)]).then(function(carddata){
                ...
                for (let j=0; j < carddata.length; j++){
                    trr_val += (total[j+1] - staked[j+1]) * convertSecToDay(carddata[j]["minStakePeriod"]) * (carddata[j]["dailyInterestRate"]/1000000);
                    fullStakeVal += parseInt(remove18(carddata[j]["maxStakeAmt"]));
                }
                document.getElementById("trr").innerHTML = formatNum(Math.round(trr_val)) + " KRED";
            }).catch(function(err){
    ...
    let populateCard = async function(num){
        return new Promise((resolve, reject) => {
            ...
            Promise.all([
                ...
                document.getElementById("apy"+num).value = Math.round((apy[num] * 365)/12500) + "%";
